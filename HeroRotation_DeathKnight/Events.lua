...
HL:RegisterForSelfCombatEvent(function(_, _, _, _, _, _, _, _, _, _, _, spellId)
  ...
  if Player:HasTier(31, 4) and (DeathKnight.GhoulTable.ApocMagusExpiration > 0 or DeathKnight.GhoulTable.ArmyMagusExpiration > 0) then
    if spellId == 85948 then
      if DeathKnight.GhoulTable:ApocMagusActive() then
        DeathKnight.GhoulTable.ApocMagusExpiration = DeathKnight
            .GhoulTable.ApocMagusExpiration + 1
      end
      if DeathKnight.GhoulTable:ArmyMagusActive() then
        DeathKnight.GhoulTable.ArmyMagusExpiration = DeathKnight
            .GhoulTable.ArmyMagusExpiration + 1
      end
    end
    for _, spell in pairs(OneRuneSpenders) do
      if spell == spellId then
        if DeathKnight.GhoulTable:ApocMagusActive() then
          DeathKnight.GhoulTable.ApocMagusExpiration = DeathKnight.GhoulTable.ApocMagusExpiration +
              0.5
        end
        if DeathKnight.GhoulTable:ArmyMagusActive() then
          DeathKnight.GhoulTable.ArmyMagusExpiration = DeathKnight.GhoulTable.ArmyMagusExpiration +
              0.5
        end
      end
    end
